...
class YoutubeVideo(BaseModel):
    ...
    video_id: Annotated[
        str, Field(..., min_length=1, max_length=25, description="YouTube video ID")
    ]
...
class CiscoYouTubeDataLoader:
            ...
            for i in range(0, len(video_ids), 50):
                ...
                for video in response["items"]:
                    ...
                    if self._has_previously_scraped_video(video_id=id):
                        ...
                        print(
                            f"Updated stale video data for video ID: {id} - data: {stale_video_data}"
                        )
    ...
    def get_video_transcript(self, video_data: List[YoutubeVideo]):
        ...
        for video in videos_data:
            ...
            if (
                ...
                and "Error:" not in video["transcript"]
            ):
                videos.append(video)
            ...
            try:
                ...
                video["transcript"] = "Error: Video Unavailable"
                ...
                video["transcript"] = "Error: Request Blocked"
    ...
    def resolve_to_series(self, videos: List[YoutubeVideo]):
        ...
        for video in videos_data:
            ...
            if any(word in tags for word in catalyst_1300x_series) or any(
                word in title
                for word in [
                    "catalyst 1300x",
                    "c1300x",
                ...
                if "series" not in video:
                    video["series"] = []
                video["series"].append("Cisco Catalyst 1300X Series Switches")
            ...
            cbs110_series = ["cbs110", "110", "unmanaged"]
            ...
            cbs220_series = ["cbs220", "cisco business 220"]
            ...
            if any(word in tags for word in smb350x_series):
                if "series" not in video:
                    video["series"] = []
                video["series"].append("Cisco 350X Series Stackable Managed Switches")
            ...
            if any(word in tags for word in smb550x_series):
                if "series" not in video:
                    video["series"] = []
                video["series"].append("Cisco 550X Series Stackable Managed Switches")
            ...
            if any(word in tags for word in rv160_series) or any(
                word in title for word in ["rv160/rv260"]
            ):
                if "series" not in video:
                    video["series"] = []
            ...
            if any(word in tags for word in rv260_series) or any(
                word in title for word in ["rv260", "rv260w", "rv160/rv260"]
            ):
                if "series" not in video:
                    video["series"] = []
            ...
            if any(word in tags for word in rv340_series) or any(
                word in title for word in ["rv340", "rv34x", "rv345p"]
            ):
                if "series" not in video:
                    video["series"] = []
